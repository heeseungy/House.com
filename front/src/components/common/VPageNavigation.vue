...
const total = ref(props.totalPage);

watchEffect(() => {
  total.value = props.totalPage;
});

const size = Math.floor(total.value % 5 > 0 ? total.value / 5 + 1 : total.value / 5);
...
// const curPageListSize = len(noticies);
...
const endPage = computed(() => {
  ...
  return total.value < lastPage ? total.value : lastPage;
});

const endRange = computed(() => {
  return parseInt((total.value - 1) / navigationSize) * navigationSize < props.currentPage;
});
